...
#«»±˖˗‑‐‒ ━ ✚‐↔←↑↓→↭⇎⇔⋆━◂▸◄►◆☀★☗☊✔✖❮❯⚑⚙
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    if [[ $has_diverged == false && $commits_behind -gt 0 ]]; then to_pull=" $fg_bold[magenta]↓$commits_behind$fg_no_bold[$fgclr]"; fi
...
prompt_time() {
  prompt_segment blue white "%{$fg_no_bold[white]%}%D{%H:%M}%{$fg_no_bold[white]%}"
}
